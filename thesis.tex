...
exact dependencies and which version of the tool chain were used. From this
...
build originally took place, and we provide several improvements to \emph{\fb},
...
not suddenly run against a newer version, this has multiple issues. First it is
...
to build one dependency or the final program and they specify how to fetch the
necessary build dependencies. The manifest also includes which runtime and SDK
need to be used and which permissions need to be applied to the resulting \fp.
Flathub, the main repository for \fp, is a community based project, where each
application has a GitHub repository under the \fh organization. These
...
were rebuilt in \autoref{sec:theo-repro}. To perform the analysis we
